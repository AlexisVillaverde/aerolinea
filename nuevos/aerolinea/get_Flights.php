...
<head>
    ...
    <style>
        ...
        .btn-clase,.btn-reservar {
            background-color: #004274 !important;
            color: white !important;
            font-weight: bold;
        }
         .btn-clase,.btn-reservar:hover {
...
<?php if (!empty($flights)) : ?>
    <?php foreach ($flights as $index => $flight) : ?>
        <div class="flight-card" id="<?= htmlspecialchars($flight['id_vuelo']) ?>">
            <div class="flight-info">
                ...
                <div>
                    ...
                    <button class="btn btn-reservar" onclick="reservar('<?= $index?>', '<?= htmlspecialchars($flight['id_vuelo']) ?>')">Reservar</button>
                </div>
                <div>
                    <h5><strong>Costo</strong></h5>
                    <p><?= htmlspecialchars($flight['Costo']) ?> USD</p>
                </div>
            </div>
            ...
            <div class="flight-class"  id="flight-class-<?= $index ?>" style="display: none">
                <div class="flight-class-item turistica">
                    ...
                    <button class="btn btn-clase"
                            onclick="reservarClase('<?= htmlspecialchars($flight['id_vuelo']) ?>', 'Clase Turísta', '<?= htmlspecialchars($flight['Fecha_Salida']) ?>', '<?= htmlspecialchars($flight['ciudad_origen']) ?>', '<?= htmlspecialchars($flight['ciudad_destino']) ?>')">
                        Reservar
                    </button>
                </div>
                ...
                <div class="flight-class-item ejecutiva">
                    ...
                    <button class="btn btn-clase"
                            onclick="reservarClase('<?= htmlspecialchars($flight['id_vuelo']) ?>', 'Clase Ejecutiva', '<?= htmlspecialchars($flight['Fecha_Salida']) ?>', '<?= htmlspecialchars($flight['ciudad_origen']) ?>', '<?= htmlspecialchars($flight['ciudad_destino']) ?>')">
                        Reservar
                    </button>
                </div>
                <div class="flight-class-item primera">
                    ...
                    <button class="btn btn-clase"
                            onclick="reservarClase('<?= htmlspecialchars($flight['id_vuelo']) ?>', 'Primera Clase', '<?= htmlspecialchars($flight['Fecha_Salida']) ?>', '<?= htmlspecialchars($flight['ciudad_origen']) ?>', '<?= htmlspecialchars($flight['ciudad_destino']) ?>')">
                        Reservar
                    </button>
                </div>
            </div>
...
<script>
    ...
    function reservarClase(index, clase, fecha, origen, destino) {
        ...
        window.location.href = `seleccion_asiento.php?${params.toString()}`;
    }
